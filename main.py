print("BOOT VERSION: 2026-06-03-26")
...
async def init_db():
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        # migrations: add missing columns without losing data
        ...
        # items.preco_base (legacy DBs)
        await _ensure_column("items", "preco_base", "ALTER TABLE items ADD COLUMN preco_base INTEGER NOT NULL DEFAULT 0")
        # backfill preco_base from preco when it is 0
        await db.execute("UPDATE items SET preco_base = preco WHERE (preco_base IS NULL OR preco_base = 0) AND preco IS NOT NULL")
...
async def seed_initial_data():
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        _ACTIVE_ITEM_IDS = {iid for (iid, _p, _e) in INITIAL_SHOP_ACTIVE}

        def _guess_loja(it: dict) -> str:
            # regra simples e segura (ajuste se quiser)
            tipo = (it.get("tipo") or "").lower()
            if tipo in ("arma", "armas", "weapon"):
                return "ferreiro"
            if tipo in ("armadura", "armaduras", "armor"):
                return "alfaiate"
            if tipo in ("magia", "spell", "grimorio"):
                return "arcano"
            return "mercador"
        # itens
        for item_id, it in INITIAL_ITEMS.items():
            ...
            if exists:
                continue
            await db.execute("""
                INSERT INTO items (item_id, nome, preco_base, preco, tipo, slot, bonus_json, efeito_json, classes_json, desc, loja, ativo, deleted)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
            """, (
                item_id,
                it["nome"],
                int(it["preco"]),
                int(it["preco"]),  # preco atual inicia igual ao preco_base
                it["tipo"],
                it["slot"],
                jdump(it.get("bonus", {})),
                jdump(it.get("efeito", {})),
                ...
                it.get("desc", "") or "",
                _guess_loja(it),
                1 if item_id in _ACTIVE_ITEM_IDS else 0,
            ))
